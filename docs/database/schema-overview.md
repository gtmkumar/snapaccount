...
- **Why (3) — `financial_year` widen:** the GAP-043 chat-thread-PDF flow (`Finance.WebApi/Endpoints/Report/Reports.cs` + `Infrastructure/Reports/ChatThreadPdfGenerator.cs`) **encodes a 36-char UUID thread id in `financial_year`** by design — reusing the report job schema without bespoke DDL. A 36-char value overflows `varchar(10)` (`22001`); `varchar(40)` fits the UUID + headroom while staying bounded.
