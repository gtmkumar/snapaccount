...
- **RV-02 (MEDIUM):** Fail-fast guards confirmed in both `AuthService.Api/Program.cs` and `AiService.Api/Program.cs`. Threshold: 32 chars. Exception type: `InvalidOperationException`. Non-Development only.
