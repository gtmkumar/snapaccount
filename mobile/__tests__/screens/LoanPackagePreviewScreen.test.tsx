...
describe('LoanPackagePreviewScreen — watermark and disclaimer', () => {
  ...
  it('watermark text is rendered in PDF viewer after bio passes', async () => {
    ...
    if (watermark) {
      // children may be a string or a mixed array [Icon, Text] — stringify for match
      const childText = Array.isArray(watermark.props.children)
        ? watermark.props.children.map((c: unknown) =>
            typeof c === 'string' ? c : (c as { props?: { children?: unknown } })?.props?.children ?? '',
          ).join('')
        : String(watermark.props.children ?? '');
      expect(childText).toMatch(/Generated by SnapAccount/);
    } else {
